      ...
      <main className="catalog-main">
        ...
        <section className="product-browser">
          <aside className={`product-filters ${mobileFilters ? "is-open" : ""}`}>
            ...
            <span>{translate("Product families")}</span>
                ...
                <div className="filter-group-heading">
                  <button className={
                    normalizeCategory(category) === normalizeCategory(group.name)
                      ...
                      <span>{translate(group.name)}</span><b>{categoryCount(group.name)}</b>
                  </button>
            ...
            <button className="mobile-filter-results" type="button" onClick={() => setMobileFilters(false)}>{translate("View all products")} · {results.length}</button>
          </aside>

          <div className="product-results">
            <div className="results-toolbar">
              <div>
                <span className="micro-label">{translate("Selected collection")}</span>
                <h2>{heading}</h2>
                <p>{translate(`${results.length} product groups`)}</p>
              </div>
              <div className="results-actions">
                <label>
                  ...
                  <input
                    ...
                    placeholder={translate("Search model or product")}
                  />
                </label>
                <button className="mobile-filter-button" onClick={() => setMobileFilters(true)}>
                  <SlidersHorizontal size={17} /> {translate("Filters")}
                </button>
                <label className="sort-select" aria-label="Sort products">
                  <select value={sort} onChange={(event) => setSort(event.target.value)}>
                    <option value="catalogue">{translate("Catalogue order")}</option>
            ...
            {(category || query) && (
              <div className="active-product-filters">
                ...
                {category && <button onClick={() => chooseCategory("")}>{translate(category)}<X size={13} /></button>}
            ...
            {results.length ? (
              ...
              <div className="empty-results">
                <span>0 results</span>
                <h3>{query ? "No products match your search." : "No published products in this collection."}</h3>
                <p>
                  {query
                    ? "Try another model number or remove the current filters."
                    : "Contact our team if you need products from this category."}
                </p>
